...
const credit = ref(null); const history = ref([]); const payments = ref([]); const applications = ref([]); const operatorNames = ref({})
...
const sourceLabel = (value) => ({ MANUAL: '手动调整', RULE: '自动规则调整', RULE_CAP: '自动规则调整', INITIAL: '初始值', SYSTEM: '系统调整' })[String(value || '').toUpperCase()] || (value ? '其他来源' : '—')
function rememberOperatorNames(users) { for (const user of users || []) { if (user?.user_id) operatorNames.value[user.user_id] = String(user.username || user.account_name || user.display_name || '').trim() || '未命名用户' } }
async function resolveOperatorNames(historyItems) {
  const IDs = [...new Set(historyItems.map((item) => item?.operator_id).filter((id) => id && id !== 'system' && !operatorNames.value[id]))]
  await Promise.all(IDs.map(async (id) => { try { const result = await listOwnerDirectory({ user_id: id, page: 1, page_size: 1 }); rememberOperatorNames(result?.items || []) } catch { /* 目录不可用时不向业务用户暴露内部操作人 ID。 */ } }))
}
...
// 发起调整是销售创建人权限；审批角色即使拥有同名 catalog permission，也只能处理待办。
const canApplyPermission = computed(() => props.canApply === true)
...
async function loadCustomerCredit() {
  ...
  try {
    ...
    credit.value = summary; history.value = unwrapItems(changes); payments.value = unwrapItems(records); await resolveOperatorNames(history.value)
...
<template>
  ...
  <section v-else class="crm-credit-panel" aria-labelledby="customer-credit-title">
    ...
    <p v-if="loading">正在加载信用记录…</p><template v-else>
      <div class="crm-credit-counters"><span>连续按时：<strong>{{ credit?.consecutive_ontime_count ?? 0 }}</strong></span><span>连续逾期：<strong>{{ credit?.consecutive_late_count ?? 0 }}</strong></span><span>规则来源：{{ sourceLabel(credit?.credit_change_source || 'INITIAL') }}</span></div>
      <details open><summary>等级变更历史</summary><p v-if="!history.length" class="crm-empty compact">暂无变更记录</p><ul v-else><li v-for="item in history" :key="item.id"><strong>{{ levelLabel(item.from_level) }} → {{ levelLabel(item.to_level || item.credit_level) }}</strong> · {{ sourceLabel(item.source || item.change_source) }} · {{ item.reason || item.rule_result || '—' }}<small>操作人：{{ operatorLabel(item) }} · {{ formatCreditDate(item.occurred_at) }}<template v-if="item.payment_id"> · 回款号：{{ item.payment_id }}</template><template v-if="item.application_id"> · 申请单：#{{ item.application_id }}</template></small></li></ul></details>
