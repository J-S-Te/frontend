...
const levelLabel = (value) => ({ A: 'A · 优秀', B: 'B · 正常', C: 'C · 关注', D: 'D · 高风险' })[value] || value || '未设置'
...
async function submitApply() {
  ...
  if (form.value.reason.trim().length < 20) return emit('error', '调整原因至少填写 20 个字符。')
...
<template>
  ...
  <section v-else class="crm-credit-panel" aria-labelledby="customer-credit-title">
    ...
    <p v-if="risk" class="crm-alert warning" role="status">该客户信用等级为 {{ currentLevel }}，请关注回款与交易风险。等级提示不阻断业务流程。</p>
    <p v-if="loading">正在加载信用记录…</p><template v-else>
      ...
      <details open><summary>等级变更历史</summary><p v-if="!history.length" class="crm-empty compact">暂无变更记录</p><ul v-else><li v-for="item in history" :key="item.id">{{ item.from_level || '—' }} → {{ item.to_level || item.credit_level }} · {{ item.source || item.change_source }} · {{ item.reason || item.rule_result || '—' }}</li></ul></details>
      <details><summary>回款评估记录</summary><p v-if="!payments.length" class="crm-empty compact">暂无回款评估记录</p><ul v-else><li v-for="item in payments" :key="item.id">{{ item.payment_id || item.receipt_id }} · {{ item.evaluation_result || item.result || '未参与评估' }} · 宽限期 {{ item.grace_days_applied ?? '—' }} 天</li></ul></details>
    </template>
    <div v-if="showForm" class="crm-credit-apply-form"><h3>申请调整信用等级</h3><label>目标等级<select v-model="form.target_level"><option value="" disabled>请选择</option><option v-for="item in levels" :key="item" :value="item" :disabled="item === currentLevel">{{ levelLabel(item) }}</option></select></label><label>调整原因（至少 20 字）<textarea v-model.trim="form.reason" minlength="20" maxlength="500" rows="4"></textarea></label><div class="crm-actions"><button type="button" @click="showForm = false">取消</button><button type="button" class="primary" :disabled="submitting" @click="submitApply">{{ submitting ? '提交中…' : '提交申请' }}</button></div></div>
  </section>
