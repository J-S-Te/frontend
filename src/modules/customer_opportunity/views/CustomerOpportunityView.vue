...
const teamForm = reactive({ members_text: '', reason: '' })
...
function navigatePlatform() {
  mobileMenuOpen.value = false
  return router.push({ name: 'portal' })
}
...
async function openReports() { showReport.value = true; await loadReports() }
...
function openNewCustomer() { Object.assign(customerForm, emptyCustomer()); customerEditMode.value = false; customerDialog.value = true }
...
function openNewOpportunity() { Object.assign(opportunityForm, emptyOpportunity()); opportunityEditMode.value = false; opportunityDialog.value = true }
...
async function loadCustomerMergeTarget() {
  ...
  if (!Number.isSafeInteger(id) || id <= 0 || id === selectedCustomer.value?.id) {
    error.value = '请输入不同于源客户的有效目标客户 ID。'; return
  }
  try {
...
function openOpportunityPresaleCreate() {
  ...
  if (!presaleRequestSubmissionAvailable.value) {
    error.value = '售前投递 Worker 没有新鲜运行证据，申请入口已安全关闭。'
    return
  }
  ...
  presaleOpportunityLocked.value = true
...
function openTeamEditor() {
  teamForm.members_text = opportunityTeam.value.map((item) => `${item.user_id},${item.role}`).join('\n')
  teamForm.reason = ''; teamDialog.value = true
}
...
async function submitTeam() {
  try {
    const members = teamForm.members_text.split('\n').map((line) => line.trim()).filter(Boolean).map((line) => {
      const [user_id, role] = line.split(',').map((item) => item.trim())
      return { user_id, role }
    })
    ...
    selectedOpportunity.value.version = team.version; opportunityTeam.value = team.members || []
...
async function changeCustomerStatus(action) {
  const reason = window.prompt(action === 'void' ? '请输入作废原因' : '请输入恢复原因')
  if (!reason) return
  try {
    selectedCustomer.value = action === 'void'
      ? await voidCustomer(selectedCustomer.value.id, { version: selectedCustomer.value.version, reason })
      : await restoreCustomer(selectedCustomer.value.id, { version: selectedCustomer.value.version, reason })
    notice.value = action === 'void' ? '客户已作废。' : '客户已恢复。'; await loadCurrent()
  } catch (value) { showError(value) }
}
...
async function changeOpportunityStatus(action) {
  const reason = window.prompt(action === 'void' ? '请输入商机作废原因' : '请输入商机恢复原因')
  if (!reason) return
  try {
    selectedOpportunity.value = action === 'void'
      ? await voidOpportunity(selectedOpportunity.value.id, { version: selectedOpportunity.value.version, reason })
      : await restoreOpportunity(selectedOpportunity.value.id, { version: selectedOpportunity.value.version, reason })
    notice.value = action === 'void' ? '商机已作废。' : '商机已恢复。'; await loadCurrent()
  } catch (value) { showError(value) }
}
...
async function runPresale(action) {
  ...
  if (mutationActions.has(action) && (!authoritative || !Number.isInteger(version) || !authoritativePresaleActions.value.includes(({ approve: 'APPROVE', reject: 'REJECT', cancel: 'CANCEL', assign: 'ASSIGN', progress: 'ADD_PROGRESS', worklog: 'ADD_WORKLOG' })[action]))) {
  ...
  try {
    ...
    if (action === 'approve' || action === 'reject') {
      const rawComment = window.prompt(action === 'reject' ? '请输入驳回原因' : '请输入审批意见（可留空）', '')
      if (rawComment === null) return
      const comment = rawComment.trim()
      if (action === 'reject' && !comment) { error.value = '驳回原因必填。'; return }
    ...
    if (action === 'cancel') {
      const rawReason = window.prompt('请输入取消原因')
      if (rawReason === null) return
      const reason = rawReason.trim()
      if (!reason) return
      const retry = presaleMutationRetries.keyFor('cancel', id, { reason, version })
...
<template>
  <div class="console-page crm-shell">
    ...
    <main class="console-main crm-main">
      ...
      <section v-if="activeSection === 'customers'" class="crm-panel table-panel"><p v-if="loading">正在加载…</p><table v-else-if="customers.length && customerFilters.view === 'table'"><thead><tr><th>客户编号</th><th>客户名称</th><th>类型</th><th>行业 / 区域</th><th>负责人</th><th>状态</th><th>最近跟进</th><th>商机金额汇总</th></tr></thead><tbody><tr v-for="item in customers" :key="item.id" @click="openCustomer(item.id)"><td>{{ item.customer_no }}</td><td>{{ item.name }}</td><td>{{ item.customer_type }}</td><td>{{ item.industry }} / {{ item.region }}</td><td>{{ item.owner_user_id }}</td><td>{{ item.status }}</td><td>{{ formatDate(item.last_followup_at) }}</td><td>¥ {{ item.opportunity_amount_sum }}</td></tr></tbody></table><div v-else-if="customers.length" class="crm-customer-cards"><button v-for="item in customers" :key="item.id" @click="openCustomer(item.id)"><strong>{{ item.name }}</strong><span>{{ item.customer_no }} · {{ item.status }}</span><span>{{ item.customer_type }} · {{ item.industry }} / {{ item.region }}</span><span>负责人：{{ item.owner_user_id }}</span><span>最近跟进：{{ formatDate(item.last_followup_at) }}</span><span>商机金额汇总：¥ {{ item.opportunity_amount_sum }}</span></button></div><div v-else class="crm-empty">暂无符合条件的客户</div></section>
      <section v-if="activeSection === 'opportunities' && !boardMode" class="crm-panel table-panel"><p v-if="loading">正在加载…</p><table v-else-if="opportunities.length"><thead><tr><th>商机编号</th><th>名称</th><th>客户 ID</th><th>预计金额</th><th>阶段</th><th>状态</th><th>终态待办</th></tr></thead><tbody><tr v-for="item in opportunities" :key="item.id" @click="openOpportunity(item.id)"><td>{{ item.opportunity_no }}</td><td>{{ item.name }}</td><td>{{ item.customer_id }}</td><td>{{ item.expected_amount }}</td><td>{{ item.current_stage }}</td><td>{{ item.opp_status }}</td><td>{{ item.terminal_pending_type || '—' }}</td></tr></tbody></table><div v-else class="crm-empty">暂无符合条件的商机</div></section>
    ...
    <div v-if="showReport" class="crm-modal"><article class="crm-detail crm-report"><h2>售前投入报表</h2><p class="crm-note">全部工时为标准小时；时间筛选发送 RFC3339 并由服务端统一换算为 UTC 半开区间。成功率字段单位为百分比。</p><form class="crm-report-filters" @submit.prevent="loadReports"><label>开始时间<input v-model="reportFilters.from" type="datetime-local" required></label><label>结束时间<input v-model="reportFilters.to" type="datetime-local" required></label><label>组织 ID<input v-model.trim="reportFilters.organization_id"></label><label>人员 ID<input v-model.trim="reportFilters.person_id"></label><label>商机 ID<input v-model.trim="reportFilters.opportunity_id" inputmode="numeric"></label><label>分布维度<select v-model="reportFilters.dimension"><option value="PERSON">人员</option><option value="DEPARTMENT">部门</option><option value="OPPORTUNITY">商机</option></select></label><button class="primary" :disabled="reportLoading">{{ reportLoading ? '加载中…' : '查询' }}</button><button type="button" @click="exportReport">导出</button></form><div v-if="reportSummary" class="crm-kpis"><div><span>投入小时</span><strong>{{ reportSummary.work_hours }}</strong></div><div><span>参与人数</span><strong>{{ reportSummary.participant_count }}</strong></div><div><span>自动完成任务</span><strong>{{ reportSummary.auto_completed_task_count }}</strong></div><div><span>商机覆盖率</span><strong>{{ reportSummary.opportunity_coverage_rate_percent }}%</strong><small>{{ reportSummary.covered_opportunity_count }} / {{ reportSummary.active_opportunity_count }}</small></div><div><span>PMS 最终成功率</span><strong>{{ reportSummary.pms_success_rate_percent }}%</strong><small>{{ reportSummary.pms_success_count }} / {{ reportSummary.pms_outbox_worklog_count }}</small></div></div><section class="crm-report-chart"><h3>投入分布</h3><div v-for="item in reportDistribution" :key="item.dimension_id" class="crm-report-bar"><span>{{ item.dimension_name }}</span><i :style="{ width: reportBarWidth(item.work_hours) }"></i><strong>{{ item.work_hours }} 小时</strong></div><p v-if="!reportDistribution.length" class="crm-note">当前筛选没有投入记录。</p></section><section class="table-panel"><h3>趋势数值表（UTC 日）</h3><table><thead><tr><th>日期</th><th>工时</th><th>参与人数</th><th>工时记录</th></tr></thead><tbody><tr v-for="item in reportTrend" :key="item.date"><td>{{ item.date }}</td><td>{{ item.work_hours }}</td><td>{{ item.participant_count }}</td><td>{{ item.worklog_count }}</td></tr></tbody></table></section><section class="table-panel"><h3>分布数值表</h3><table><thead><tr><th>维度</th><th>部门</th><th>工时</th><th>人数</th><th>任务</th><th>工时记录</th></tr></thead><tbody><tr v-for="item in reportDistribution" :key="`row-${item.dimension_id}`"><td>{{ item.dimension_name }}</td><td>{{ item.department || '—' }}</td><td>{{ item.work_hours }}</td><td>{{ item.participant_count }}</td><td>{{ item.request_count }}</td><td>{{ item.worklog_count }}</td></tr></tbody></table></section><div class="crm-actions"><button @click="showReport = false">关闭</button></div></article></div>

    <div v-if="customerDialog" class="crm-modal" role="dialog" aria-modal="true"><form @submit.prevent="submitCustomer"><h2>{{ customerEditMode ? '编辑客户' : '新建客户' }}</h2><label>客户名称<input v-model="customerForm.name" required></label><label>统一社会信用代码<input v-model="customerForm.unified_credit_code" :placeholder="customerEditMode ? '留空表示保持原值' : ''"></label><label>客户类型<input v-model="customerForm.customer_type" required></label><label>行业<input v-model="customerForm.industry" required></label><label>区域<input v-model="customerForm.region" required></label><OwnerSelector v-model:user-id="customerForm.owner_user_id" v-model:organization-id="customerForm.owner_org_id" :default-user-id="crmSession?.user_id || ''" :default-organization-id="crmSession?.primary_org_id || ''" /><fieldset v-for="(contact, index) in customerForm.contacts" :key="contact.id || index"><legend>联系人 {{ index + 1 }}</legend><label>姓名<input v-model="contact.name" required></label><label>电话<input v-model="contact.phone" :required="!contact.id" :placeholder="contact.id ? '留空表示保持原值' : ''"></label><label>邮箱<input v-model="contact.email" type="email" :placeholder="contact.id ? '留空表示保持原值' : ''"></label><label class="check"><input v-model="contact.is_registration" type="checkbox">门户登记联系人</label></fieldset><button type="button" @click="addCustomerContact">添加联系人</button><label>{{ customerEditMode ? '更新原因' : '创建原因' }}<textarea v-model="customerForm.reason" required></textarea></label><label class="check"><input v-model="customerForm.duplicate_override" type="checkbox">授权覆盖名称查重</label><label v-if="customerForm.duplicate_override">覆盖原因<textarea v-model="customerForm.duplicate_override_reason" required></textarea></label><div class="crm-actions"><button type="button" @click="customerDialog = false">取消</button><button class="primary" :disabled="actionLoading">保存</button></div></form></div>
    ...
    <div v-if="opportunityDialog" class="crm-modal"><form @submit.prevent="submitOpportunity"><h2>{{ opportunityEditMode ? '编辑商机' : '新建商机' }}</h2><label>商机名称<input v-model="opportunityForm.name" required></label><label>客户 ID<input v-model="opportunityForm.customer_id" required inputmode="numeric" :disabled="opportunityEditMode"></label><label>商机类型<input v-model="opportunityForm.type" required></label><label>来源<input v-model="opportunityForm.source" required></label><label>预计金额<input v-model="opportunityForm.expected_amount" required></label><label>预计签单日期<input v-model="opportunityForm.expected_sign_date" type="date" required></label><label>需求摘要<textarea v-model="opportunityForm.requirement_summary" required></textarea></label><label>系统数量<input v-model.number="opportunityForm.system_count" type="number" min="0"></label><label>客户痛点<textarea v-model="opportunityForm.pain_points"></textarea></label><label>竞争信息<textarea v-model="opportunityForm.competitor_info"></textarea></label><OwnerSelector v-if="!opportunityEditMode" v-model:user-id="opportunityForm.owner_user_id" v-model:organization-id="opportunityForm.owner_org_id" :default-user-id="crmSession?.user_id || ''" :default-organization-id="crmSession?.primary_org_id || ''" /><fieldset v-else class="crm-owner-selector" disabled><legend>负责人</legend><label>负责人用户<input :value="opportunityForm.owner_user_id" readonly></label><label>负责人组织<input :value="opportunityForm.owner_org_id" readonly></label><p class="crm-note">商机主档编辑不变更负责人，请使用详情中的“变更负责人”。</p></fieldset><label v-if="opportunityEditMode">更新原因<textarea v-model="opportunityForm.reason" required></textarea></label><div class="crm-actions"><button type="button" :disabled="actionLoading" @click="opportunityDialog = false">取消</button><button class="primary" :disabled="actionLoading">{{ actionLoading ? '提交中…' : opportunityEditMode ? '保存' : '创建' }}</button></div></form></div>
...
<dt>存续客户 ID</dt>
<dd>{{ selectedCustomer.merged_into_id }}</dd>
<dt>合并日期</dt>
...
<dt>负责人</dt>
<dd>{{ selectedCustomer.owner_user_id }}</dd>
<dt>版本</dt>
...
<p v-for="contact in customerContacts" :key="contact.id">{{ contact.name }} · {{ contact.phone }} · {{ contact.email || '—' }}<span v-if="contact.is_registration"> · 登记联系人</span>
</p>
</section>
...
<td>{{ item.current_stage }} / {{ item.opp_status }}</td>
</tr>
</tbody>
</table>
</section>
...
<ol>
<li v-for="item in customerFollowups" :key="item.id">
<strong>{{ item.type }}</strong> · {{ formatDate(item.followed_at) }}<br>{{ item.content }}</li>
</ol>
</section>
<section v-if="customerTab === 'audit' && !customerTabErrors.audit">
<p v-if="!customerAuditLogs.length">暂无操作日志</p>
<ol>
<li v-for="item in customerAuditLogs" :key="item.id">
<strong>{{ item.operation }}</strong> · {{ formatDate(item.occurred_at) }} · {{ item.actor_id }} · {{ item.result }}<br>{{ item.reason || '—' }}</li>
</ol>
</section>
<div class="crm-actions">
...
<button v-else-if="selectedCustomer.status === 'VOID'" class="primary" @click="changeCustomerStatus('restore')">恢复</button>
    ...
    <div v-if="customerMergeDialog && selectedCustomer" class="crm-modal nested"><form @submit.prevent="submitCustomerMerge"><h2>客户合并向导</h2><p class="crm-note">源客户：{{ selectedCustomer.name }}（{{ selectedCustomer.customer_no }}，版本 {{ selectedCustomer.version }}）。目标客户作为存续主档；提交后源客户进入 MERGED，本期不支持反合并。</p><label>目标客户 ID<input v-model.trim="customerMergeForm.target_customer_id" inputmode="numeric" required></label><button type="button" @click="loadCustomerMergeTarget">加载并核对目标</button><dl v-if="customerMergeForm.target" class="crm-merge-compare"><dt>目标名称</dt><dd>{{ customerMergeForm.target.name }}</dd><dt>目标编号</dt><dd>{{ customerMergeForm.target.customer_no }}</dd><dt>行业 / 区域</dt><dd>{{ customerMergeForm.target.industry }} / {{ customerMergeForm.target.region }}</dd><dt>负责人</dt><dd>{{ customerMergeForm.target.owner_user_id }}</dd><dt>目标版本</dt><dd>{{ customerMergeForm.target.version }}</dd></dl><p class="crm-note">服务端会迁移已确认的 CRM 联系人、沟通、无合同商机和邀请。存在 Portal 身份映射、待补偿的外部开通任务或已关联合同商机时会安全阻断，不会返回假成功。</p><label>合并原因<textarea v-model.trim="customerMergeForm.reason" required maxlength="500"></textarea></label><div class="crm-actions"><button type="button" @click="customerMergeDialog = false">取消</button><button class="danger" :disabled="actionLoading || !customerMergeForm.target">{{ actionLoading ? '合并中…' : '确认合并' }}</button></div></form></div>
    ...
    <div v-if="selectedOpportunity" class="crm-modal"><article class="crm-detail"><h2>{{ selectedOpportunity.name }}</h2><dl><dt>商机编号</dt><dd>{{ selectedOpportunity.opportunity_no }}</dd><dt>负责人</dt><dd>{{ selectedOpportunity.owner_user_id }} / {{ selectedOpportunity.owner_org_id || '未填写组织' }}</dd><dt>当前阶段</dt><dd>{{ selectedOpportunity.current_stage }}</dd><dt>状态</dt><dd>{{ selectedOpportunity.opp_status }}</dd><dt>终态待办</dt><dd>{{ selectedOpportunity.terminal_pending_type === 'CONTRACT' ? '待关联合同' : selectedOpportunity.terminal_pending_type === 'LOST_REASON' ? '待补失败原因' : '无' }}</dd><dt>需求摘要</dt><dd>{{ selectedOpportunity.requirement_summary }}</dd><dt>系统数量</dt><dd>{{ selectedOpportunity.system_count || 0 }}</dd><dt>客户痛点</dt><dd>{{ selectedOpportunity.pain_points || '—' }}</dd><dt>竞争信息</dt><dd>{{ selectedOpportunity.competitor_info || '—' }}</dd><dt>版本</dt><dd>{{ selectedOpportunity.version }}</dd></dl><section class="crm-opportunity-presale" aria-labelledby="opportunity-presale-heading"><div class="crm-panel-heading"><div><h3 id="opportunity-presale-heading">关联售前任务</h3><p class="crm-note">售前状态独立流转，不会自动修改商机阶段。</p></div><button v-if="canCreatePresale && selectedOpportunity.opp_status !== 'VOID'" type="button" class="primary" @click="openOpportunityPresaleCreate">发起售前支持</button></div><p v-if="opportunityPresaleLoading">正在加载关联售前任务…</p><div v-else-if="opportunityPresaleError" class="crm-alert error" role="alert">{{ opportunityPresaleError }} <button type="button" @click="loadOpportunityPresales()">重试</button></div><div v-else-if="!opportunityPresales.length" class="crm-empty compact">暂无关联售前任务</div><div v-else class="table-panel"><table><thead><tr><th>任务号 / 申请时间</th><th>状态 / 紧急度</th><th>场地</th><th>当前执行人</th><th>最新进度</th><th>累计小时</th><th>期望结束</th><th>超时</th><th>详情</th></tr></thead><tbody><tr v-for="item in opportunityPresales" :key="item.id"><td>{{ item.request_no }}<br><small>{{ formatDate(item.created_at) }}</small></td><td>{{ requestStatusText(item.status) }} / {{ urgencyText(item.urgency) }}</td><td>{{ venueText(item.venue) }}</td><td>{{ assignees(item.current_assignees) }}</td><td>{{ item.latest_progress || '—' }}</td><td>{{ item.total_work_hours }} 小时</td><td>{{ formatDate(item.expected_end) }}</td><td>{{ item.overdue ? '已超时' : '否' }}</td><td><button v-if="item.can_view_detail" type="button" @click="openOpportunityPresale(item)">查看详情</button><span v-else>仅摘要</span></td></tr></tbody></table><div class="crm-actions"><button type="button" :disabled="opportunityPresalePage.number <= 1 || opportunityPresaleLoading" @click="loadOpportunityPresales(opportunityPresalePage.number - 1)">上一页</button><span>第 {{ opportunityPresalePage.number }} 页，共 {{ opportunityPresalePage.total }} 条</span><button type="button" :disabled="opportunityPresalePage.number * opportunityPresalePage.size >= opportunityPresalePage.total || opportunityPresaleLoading" @click="loadOpportunityPresales(opportunityPresalePage.number + 1)">下一页</button><button v-if="opportunityPresalePage.total > opportunityPresalePage.size && opportunityPresalePage.size < 100" type="button" @click="viewAllOpportunityPresales">查看全部（最多 100 条）</button></div></div></section><h3>商机团队</h3><p class="crm-note">成员 ID 为基础平台 sub；平台人员目录选择器与停用校验尚待稳定接口。</p><p v-if="!opportunityTeam.length">当前无辅助团队成员</p><ul><li v-for="member in opportunityTeam" :key="member.user_id">{{ member.user_id }} · {{ member.role }}</li></ul><h3>跟进记录</h3><p v-if="!followups.length">暂无记录</p><ol><li v-for="item in followups" :key="item.id"><strong>{{ item.type }}</strong> · {{ formatDate(item.followed_at) }}<br>{{ item.content }}</li></ol><h3>阶段历史</h3><ol><li v-for="item in stageHistory" :key="item.id">{{ item.from_stage }} → {{ item.to_stage }} · {{ formatDate(item.changed_at) }} · {{ item.reason }}</li></ol><div class="crm-actions"><button @click="selectedOpportunity = null">关闭</button><template v-if="selectedOpportunity.opp_status !== 'VOID'"><button @click="editOpportunity">编辑</button><button @click="openOwnerEditor">变更负责人</button><button @click="openTeamEditor">维护团队</button><button @click="followupDialog = true">添加跟进</button><button class="primary" @click="stageDialog = true">调整阶段</button><button class="danger" @click="changeOpportunityStatus('void')">作废</button><button v-if="selectedOpportunity.terminal_pending_type && selectedOpportunity.terminal_pending_type !== 'NONE'" class="primary" @click="terminalDialog = true">补全终态待办</button></template><button v-else class="primary" @click="changeOpportunityStatus('restore')">恢复</button></div></article></div>
    <div v-if="presaleCreateDialog && selectedOpportunity" class="crm-modal nested"><form @submit.prevent="submitOpportunityPresale"><h2>从商机发起售前支持</h2><p class="crm-note">商机已由当前详情预填并锁定；提交售前申请不会修改商机阶段。</p><label>商机 ID<input v-model="presaleForm.opportunity_id" inputmode="numeric" required :readonly="presaleOpportunityLocked"></label><label>支持方式<select v-model="presaleForm.venue"><option value="REMOTE">远程</option><option value="ONSITE">现场</option></select></label><label>服务地址<input v-model.trim="presaleForm.service_address" :required="presaleForm.venue === 'ONSITE'"></label><label>联系人<input v-model.trim="presaleForm.contact_name" required></label><label>联系电话<input v-model.trim="presaleForm.contact_phone" required></label><label>需求说明<textarea v-model.trim="presaleForm.description" minlength="10" maxlength="2000" required></textarea></label><label>预计开始<input v-model="presaleForm.expected_start" type="datetime-local" required></label><label>预计结束<input v-model="presaleForm.expected_end" type="datetime-local" required></label><label>紧急程度<select v-model="presaleForm.urgency"><option value="NORMAL">普通</option><option value="URGENT">紧急</option></select></label><div class="crm-actions"><button type="button" :disabled="presaleCreateLoading" @click="presaleCreateDialog = false; presaleOpportunityLocked = false">取消</button><button class="primary" :disabled="presaleCreateLoading">{{ presaleCreateLoading ? '提交中…' : '提交申请' }}</button></div></form></div>
    ...
    <div v-if="selectedPresale" class="crm-modal"><article class="crm-detail crm-presale-detail"><h2>{{ selectedPresale.request.request_no }}</h2><dl><dt>状态</dt><dd>{{ requestStatusText(selectedPresale.request.status) }}</dd><dt>商机</dt><dd>{{ selectedPresale.request.opportunity_no }}</dd><dt>联系人</dt><dd>{{ selectedPresale.request.contact_name || '—' }}</dd><dt>联系电话</dt><dd><span v-if="presaleContactPhone">{{ presaleContactPhone }}</span><span v-else>{{ selectedPresale.request.contact_phone || '—' }}</span> <button v-if="canViewSelectedPresaleContactPhone && !presaleContactPhone" type="button" :disabled="presaleContactPhoneLoading" @click="viewPresaleContactPhone">{{ presaleContactPhoneLoading ? '查看中…' : '查看明文' }}</button><button v-if="presaleContactPhone" type="button" @click="presaleContactPhone = ''">隐藏</button><small v-if="canViewSelectedPresaleContactPhone" class="crm-note">显式查看将写入敏感信息访问审计，本页关闭后即清除。</small><small v-if="presaleContactPhoneError" class="crm-alert error" role="alert">{{ presaleContactPhoneError }}</small></dd><dt>当前执行人</dt><dd>{{ assignees(selectedPresale.current_assignees) }}</dd><dt>累计工时</dt><dd>{{ selectedPresale.total_work_hours }} 小时</dd><dt>逾期</dt><dd>{{ selectedPresale.overdue ? '是' : '否' }}</dd></dl>
      <section class="crm-presale-actions" aria-labelledby="presale-actions-heading"><h3 id="presale-actions-heading">可用操作</h3><p v-if="presaleActionsLoading" class="crm-note">正在向服务端确认可用操作…</p><p v-else-if="presaleActionsError" class="crm-alert error" role="alert">{{ presaleActionsError }} <button type="button" @click="refreshPresaleActions(selectedPresale.request.id)">重试</button></p><p v-else class="crm-note">{{ authoritativePresaleActions.map(actionText).join('、') || '当前仅可查看' }}。多人任务在所有当前执行人各有至少一笔有效工时后自动完成。</p><p v-if="selectedPresale.request.status === 'PENDING_APPROVAL'" class="crm-note">审批操作会由服务端按实例、节点和当前登录审批人实时解析权威待办；浏览器不会接收或提交内部任务 ID。正式审批引擎未配置或查询失败时，可用操作会安全关闭。</p><div class="crm-actions"><button @click="runPresale('assignments')">查看指派</button><button @click="runPresale('history')">审批历史</button><button v-if="authoritativePresaleActions.includes('APPROVE')" class="primary" :disabled="presaleMutationLoading" @click="runPresale('approve')">通过审批</button><button v-if="authoritativePresaleActions.includes('REJECT')" class="danger" :disabled="presaleMutationLoading" @click="runPresale('reject')">驳回审批</button><button v-if="authoritativePresaleActions.includes('CANCEL')" class="danger" @click="runPresale('cancel')">取消申请</button><button v-if="authoritativePresaleActions.includes('ASSIGN')" @click="openEngineerPicker">选择或改派执行人</button></div></section>
      <template v-if="authoritativePresaleActions.includes('ADD_PROGRESS')"><h3>登记进度</h3><label>进度说明<textarea v-model.trim="operation.progress" minlength="1" maxlength="2000"></textarea></label><label>参考链接（仅 HTTPS）<input v-model.trim="operation.progress_link" type="url" placeholder="https://"></label><label>进度百分比<input v-model="operation.progress_pct" type="number" min="0" max="100"></label><button @click="runPresale('progress')">登记进度</button></template>
      <details v-if="authoritativePresaleActions.includes('ADD_WORKLOG')"><summary>登记工时</summary><label>开始<input v-model="operation.work_start" type="datetime-local"></label><label>结束<input v-model="operation.work_end" type="datetime-local"></label><label>单位<select v-model="operation.raw_unit"><option value="HOUR">小时</option><option value="PERSON_DAY">人天（1 人天 = 8 小时）</option></select></label><label>数值<input v-model="operation.raw_value"></label><label>工作地点<input v-model="operation.work_site_address"></label><label>工作内容<select v-model="operation.work_content"><option value="SOLUTION_DESIGN">方案设计</option><option value="TECH_EXCHANGE">技术交流</option><option value="POC_DEMO">POC 演示</option><option value="TECH_QA">技术答疑</option><option value="OTHER">其他</option></select></label><label>备注<input v-model="operation.remark"></label><button :disabled="presaleMutationLoading" @click="runPresale('worklog')">{{ presaleMutationLoading ? '提交中…' : '提交工时' }}</button></details>
      <section class="crm-presale-timeline" aria-labelledby="presale-timeline-heading"><div class="crm-subsection-heading"><div><h3 id="presale-timeline-heading">流程时间线</h3><p class="crm-note">按服务端稳定游标倒序加载，流程记录只读。</p></div><button type="button" :disabled="presaleTimelineLoading" @click="refreshPresaleTimeline(selectedPresale.request.id)">刷新</button></div><p v-if="presaleTimelineLoading && !presaleTimeline.length" class="crm-note">正在加载流程记录…</p><p v-if="presaleTimelineError" class="crm-alert error" role="alert">{{ presaleTimelineError }}</p><ol v-if="presaleTimeline.length" class="crm-timeline-list"><li v-for="item in presaleTimeline" :key="item.event_id"><time :datetime="item.occurred_at">{{ formatDate(item.occurred_at) }}</time><div><strong>{{ timelineEventText(item.type) }}</strong><span v-if="item.actor_name || item.actor_id">操作人：{{ item.actor_name || item.actor_id }}</span><span v-if="item.type === 'STATUS_CHANGED'">{{ requestStatusText(item.from_status) }} → {{ requestStatusText(item.to_status) }}</span><span v-if="item.type === 'APPROVAL_DECIDED'">审批结果：{{ approvalResultText(item.result) }}</span><span v-if="item.type === 'ASSIGNEE_ADDED' || item.type === 'ASSIGNEE_REMOVED'">执行人：{{ item.subject_name || item.subject_id }}</span><p v-if="item.content">{{ item.content }}</p><span v-if="item.progress_pct !== null && item.progress_pct !== undefined">进度：{{ item.progress_pct }}%</span><a v-if="safeHTTPSURL(item.link_url)" :href="safeHTTPSURL(item.link_url)" target="_blank" rel="noopener noreferrer">打开安全链接</a><span v-if="item.work_hours">{{ workContentText(item.work_content) }} · {{ item.work_hours }} 小时</span></div></li></ol><p v-else-if="!presaleTimelineLoading && !presaleTimelineError" class="crm-empty compact">暂无流程记录</p><button v-if="presaleTimelineCursor" type="button" :disabled="presaleTimelineLoading" @click="loadMorePresaleTimeline">{{ presaleTimelineLoading ? '加载中…' : '加载更多' }}</button></section>
      <h3>工时记录</h3><p v-if="!worklogs.length">暂无工时</p><div v-for="item in worklogs" :key="item.id" class="crm-worklog"><span>{{ item.person_name || item.person_id }} · {{ item.work_hours }} 小时 · {{ pushStatusText(item.push_status) }}</span><button @click="operation.worklog_id = String(item.id); runPresale('delivery')">投递状态</button></div><pre v-if="presaleResult">{{ JSON.stringify(presaleResult, null, 2) }}</pre><div class="crm-actions"><button @click="closePresale">关闭</button><button v-if="operation.worklog_id" @click="runPresale('retry')">重试所选工时投递</button></div></article></div>
  </div>
