...
const canDelete = computed(() => permissionSet.value.has('filing.delete'))
...
watch(dirty, (pending) => {
  // 草稿尚未落盘时，关闭/刷新页面前提醒，避免自动暂存窗口内的输入丢失。
  if (pending) window.addEventListener('beforeunload', beforeUnloadGuard)
...
const fields = Object.freeze({
  ORGANIZATION: [
    field('unit_name', '单位名称', 'text', null, { required: true, maxlength: 200, group: '基本信息' }),
    field('social_credit_code', '统一社会信用代码', 'text', null, { required: true, maxlength: 18, group: '基本信息' }), field('province', '省（自治区、直辖市）', 'text', null, { required: true, group: '基本信息' }), field('city', '地（区、市、州、盟）', 'text', null, { required: true, group: '基本信息' }), field('district', '县（区、市、旗）', 'text', null, { required: true, group: '基本信息' }), field('address', '详细地址', 'text', null, { required: true, group: '基本信息' }), field('postal_code', '邮政编码', 'text', null, { maxlength: 6, group: '基本信息' }), field('administrative_division_code', '行政区划代码', 'text', null, { maxlength: 6, group: '基本信息' }),
    field('organization_leader_name', '单位负责人姓名', 'text', null, { required: true, group: '单位负责人' }), field('organization_leader_title', '单位负责人职务/职称', 'text', null, { group: '单位负责人' }), field('organization_leader_phone', '单位负责人电话', 'text', null, { group: '单位负责人' }), field('organization_leader_email', '单位负责人邮箱', 'email', null, { group: '单位负责人' }),
    field('security_department', '网络安全责任部门', 'text', null, { required: true, group: '网络安全联系人' }), field('security_contact_name', '网络安全联系人', 'text', null, { required: true, group: '网络安全联系人' }), field('security_contact_phone', '网络安全联系人电话', 'text', null, { group: '网络安全联系人' }), field('security_contact_email', '网络安全联系人邮箱', 'email', null, { group: '网络安全联系人' }),
    field('data_security_department', '数据安全管理部门', 'text', null, { group: '数据安全联系人' }), field('data_security_contact_name', '数据安全联系人', 'text', null, { group: '数据安全联系人' }),
    field('affiliation', '隶属关系', 'select', enumOptions.affiliation, { required: true, group: '单位属性' }), field('organization_type', '单位类型', 'select', enumOptions.organizationType, { required: true, group: '单位属性' }), field('industry_code', '行业类别代码', 'text', null, { required: true, group: '单位属性' }),
    field('level2_object_count', '第二级定级对象数', 'number', null, { required: true, min: 0, group: '定级对象数量' }), field('level3_object_count', '第三级定级对象数', 'number', null, { required: true, min: 0, group: '定级对象数量' }), field('level4_object_count', '第四级定级对象数', 'number', null, { required: true, min: 0, group: '定级对象数量' }), field('level5_object_count', '第五级定级对象数', 'number', null, { required: true, min: 0, group: '定级对象数量' }),
  ],
  CLASSIFIED_OBJECT: [
    field('system_name', '定级对象（系统名称）', 'text', null, { required: true, group: '对象基本信息' }), field('object_number', '定级对象编号', 'text', null, { group: '对象基本信息' }), field('object_types', '定级对象类型', 'checks', enumOptions.objectTypes, { required: true, group: '对象基本信息' }), field('business_type', '业务类型', 'select', enumOptions.businessType, { required: true, group: '对象基本信息' }), field('business_description', '业务描述', 'textarea', null, { required: true, group: '对象基本信息' }), field('launched_on', '投入运行日期', 'date', null, { required: true, group: '对象基本信息' }),
    field('service_scope', '服务范围', 'select', enumOptions.serviceScope, { required: true, group: '服务与部署' }), field('service_audience', '服务对象', 'select', enumOptions.audience, { required: true, group: '服务与部署' }), field('deployment_scope', '部署范围', 'select', enumOptions.deployment, { required: true, group: '服务与部署' }), field('network_nature', '网络性质', 'select', enumOptions.networkNature, { required: true, group: '服务与部署' }), field('source_ip_range', '源站 IP 地址范围', 'text', null, { group: '服务与部署' }), field('domain_name', '域名', 'text', null, { group: '服务与部署' }), field('protocols_and_ports', '主要协议/端口', 'text', null, { group: '服务与部署' }), field('interconnection', '网络互联情况', 'textarea', null, { group: '服务与部署' }),
    field('is_subsystem', '是否是分系统', 'boolean', null, { required: true, group: '分系统关联' }), field('parent_system_name', '上级系统名称', 'text', null, { show: data => data.is_subsystem, group: '分系统关联' }), field('parent_organization_name', '上级系统所属单位', 'text', null, { show: data => data.is_subsystem, group: '分系统关联' }),
  ],
  CLASSIFICATION: [
    field('business_information_level', '业务信息安全保护等级', 'level', null, { required: true, group: '定级结果' }), field('system_service_level', '系统服务安全保护等级', 'level', null, { required: true, group: '定级结果' }), field('final_level', '定级对象安全保护等级', 'level', null, { required: true, group: '定级结果' }), field('classified_on', '定级时间', 'date', null, { required: true, group: '定级结果' }),
    field('classification_report_available', '已有定级报告声明', 'boolean', null, { required: true, group: '评审情况' }), field('expert_reviewed', '已通过专家评审', 'boolean', null, { required: true, group: '评审情况' }), field('has_industry_authority', '有上级行业主管部门', 'boolean', null, { required: true, group: '评审情况' }), field('industry_authority_name', '上级行业主管部门名称', 'text', null, { show: data => data.has_industry_authority, group: '评审情况' }), field('industry_authority_reviewed', '已通过上级行业主管部门审核', 'boolean', null, { required: true, group: '评审情况' }),
    field('form_filler_name', '填表人姓名', 'text', null, { required: true, group: '填表信息' }), field('form_filled_on', '填表日期', 'date', null, { required: true, group: '填表信息' }),
  ],
  NEW_TECHNOLOGY: [
    field('cloud_used', '使用云计算技术', 'boolean', null, { required: true, group: '云计算' }), field('cloud_responsibility', '云计算角色', 'select', enumOptions.cloudResponsibility, { show: data => data.cloud_used, group: '云计算' }), field('cloud_service_model', '云服务模式', 'select', enumOptions.cloudService, { show: data => data.cloud_used, group: '云计算' }), field('cloud_deployment_model', '云部署模式', 'select', enumOptions.cloudDeployment, { show: data => data.cloud_used, group: '云计算' }), field('cloud_platform_name', '云平台名称', 'text', null, { show: data => data.cloud_used, group: '云计算' }), field('cloud_platform_level', '云平台安全保护等级', 'level', null, { show: data => data.cloud_used, group: '云计算' }), field('cloud_filing_number', '云平台备案编号', 'text', null, { show: data => data.cloud_used, group: '云计算' }),
    field('mobile_used', '使用移动互联技术', 'boolean', null, { required: true, group: '移动互联' }), field('mobile_application_names', '移动应用名称', 'textarea', null, { show: data => data.mobile_used, group: '移动互联' }),
    field('iot_used', '使用物联网技术', 'boolean', null, { required: true, group: '物联网' }), field('iot_components', '物联网组成', 'checks', enumOptions.iot, { show: data => data.iot_used, group: '物联网' }),
    field('industrial_control_used', '使用工业控制技术', 'boolean', null, { required: true, group: '工业控制' }), field('industrial_control_components', '工业控制组成', 'checks', enumOptions.industrial, { show: data => data.industrial_control_used, group: '工业控制' }),
    field('big_data_used', '使用大数据技术', 'boolean', null, { required: true, group: '大数据' }), field('big_data_components', '大数据组成', 'checks', enumOptions.bigData, { show: data => data.big_data_used, group: '大数据' }), field('big_data_cross_border', '大数据涉及跨境', 'boolean', null, { show: data => data.big_data_used, group: '大数据' }), field('big_data_application_count', '大数据应用数量', 'number', null, { min: 0, show: data => data.big_data_used, group: '大数据' }), field('big_data_platform_name', '大数据平台名称', 'text', null, { show: data => data.big_data_used, group: '大数据' }),
  ],
  MATERIALS: [
    field('topology_available', '网络拓扑图', 'material', null, { fileKey: 'topology_file_name', group: '材料清单' }), field('security_governance_available', '网络安全管理制度', 'material', null, { fileKey: 'security_governance_file_name', group: '材料清单' }), field('security_design_available', '网络安全建设方案', 'material', null, { fileKey: 'security_design_file_name', group: '材料清单' }), field('security_products_available', '网络安全产品清单', 'material', null, { fileKey: 'security_products_file_name', group: '材料清单' }), field('security_services_available', '网络安全服务清单', 'material', null, { fileKey: 'security_services_file_name', group: '材料清单' }), field('authority_guidance_available', '上级主管部门指导意见', 'material', null, { fileKey: 'authority_guidance_file_name', group: '材料清单' }),
  ],
  DATA_INVENTORY: [
    field('data_name', '数据名称', 'text', null, { required: true, group: '数据基本信息' }), field('proposed_data_level', '拟定数据级别', 'select', enumOptions.dataLevel, { required: true, group: '数据基本信息' }), field('data_category', '数据类别', 'text', null, { required: true, group: '数据基本信息' }), field('responsible_department', '责任部门', 'text', null, { required: true, group: '数据基本信息' }), field('responsible_person', '责任人', 'text', null, { required: true, group: '数据基本信息' }), field('total_volume', '数据总量', 'text', null, { group: '数据基本信息' }), field('monthly_growth', '月增长量', 'text', null, { group: '数据基本信息' }),
    field('personal_information_types', '个人信息类型', 'checks', enumOptions.personalInfo, { required: true, group: '数据流转与存储' }), field('data_sources', '数据来源', 'checks', enumOptions.dataSources, { required: true, group: '数据流转与存储' }), field('inter_organization_flow', '组织间流转情况', 'textarea', null, { group: '数据流转与存储' }), field('processor_interaction', '处理者交互方式', 'select', enumOptions.processor, { required: true, group: '数据流转与存储' }), field('storage_locations', '存储位置', 'checks', enumOptions.storage, { required: true, group: '数据流转与存储' }),
  ],
  CLASSIFICATION_REPORT: [
    field('responsible_entity_description', '定级责任主体说明', 'textarea', null, { required: true, group: '定级说明' }), field('object_composition_description', '定级对象构成说明', 'textarea', null, { required: true, group: '定级说明' }), field('business_description', '承载业务说明', 'textarea', null, { required: true, group: '定级说明' }), field('subsystems_summary', '子系统概要', 'textarea', null, { group: '定级说明' }), field('data_description', '数据情况说明', 'textarea', null, { required: true, group: '定级说明' }), field('security_responsibility_description', '安全责任说明', 'textarea', null, { required: true, group: '定级说明' }),
    field('business_information_description', '业务信息受损影响说明', 'textarea', null, { required: true, group: '业务信息安全' }), field('business_impact_object', '业务信息受损侵害客体', 'select', enumOptions.impactObject, { required: true, group: '业务信息安全' }), field('business_damage_degree', '业务信息受损程度', 'select', enumOptions.damage, { required: true, group: '业务信息安全' }), field('business_information_level', '业务信息安全等级', 'level', null, { required: true, group: '业务信息安全' }),
    field('system_service_description', '系统服务受损影响说明', 'textarea', null, { required: true, group: '系统服务安全' }), field('service_impact_object', '系统服务受损侵害客体', 'select', enumOptions.impactObject, { required: true, group: '系统服务安全' }), field('service_damage_degree', '系统服务受损程度', 'select', enumOptions.damage, { required: true, group: '系统服务安全' }), field('system_service_level', '系统服务安全等级', 'level', null, { required: true, group: '系统服务安全' }),
    field('final_level', '最终安全保护等级', 'level', null, { required: true, group: '最终等级' }),
  ],
})
...
async function startFiling() {
  ...
  try {
    applyDetail(await createFiling({ project_id: createProjectId.value }, createIdempotencyKey))
    createIdempotencyKey = ''; creating.value = false; createProjectId.value = ''
    emit('notice', '备案草稿已创建。')
  }
  catch (error) { emit('error', error) }
  finally { busy.value = false }
...
async function deleteDraft(item) {
  if (!canDelete.value || item.status !== 'DRAFT' || busy.value) return
  if (!window.confirm(`确认删除备案草稿 ${item.filing_no}？删除后草稿内容与材料声明不可恢复。`)) return
  busy.value = true
  try { await deleteFiling(item.id); emit('notice', `备案草稿 ${item.filing_no} 已删除。`); filing.value = null; await loadList() }
  catch (error) { emit('error', error) }
  finally { busy.value = false }
}
...
async function confirmSubmit() {
  ...
  try {
    ...
    submitIdempotencyKey = ''; emit('notice', '备案快照已锁定，正在等待正式公安提交契约；当前不代表已向公安提交。后续请等待服务人员联系办理，如需修改请联系服务人员解锁。')
  } catch (error) {
