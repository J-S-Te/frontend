...
test('Portal report view gates read/request/download independently and cancels stale downloads', async () => {
  ...
  assert.match(view, /v-else-if="section === 'reports'" class="portal-empty">\{\{ serviceUnavailableText\('report_enabled'/)
  assert.match(view, /serviceUnavailableText\('report_enabled', '当前账号没有报告读取或申请权限/)
