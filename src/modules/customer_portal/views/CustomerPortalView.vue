...
async function load() {
  ...
  try {
    ...
    else if (section.value === 'feedback' && hasPermission('feedback.read')) { const value = await listFeedbacks({ page: 1, page_size: 20 }); feedbacks.value = value?.items || [] }
...
async function loadReportWorkspace() {
  ...
  if (hasPermission('report.read')) {
    ...
    requests.push(listReportRequests({ page: 1, page_size: 20 }).then((value) => {
      if (generation === reportLoadGeneration && section.value === 'reports') reports.value = value?.items || []
    }).catch((value) => {
...
async function submitProjectEvaluation() { if (!evaluationReady.value || !selectedProject.value) return; error.value = ''; notice.value = ''; try { evaluation.value = await submitEvaluation({ project_id: selectedProject.value.snapshot.project_id, ...evaluationForm }); evaluationEligibility.value = { project_id: selectedProject.value.snapshot.project_id, eligible: false, reason_code: 'ALREADY_EVALUATED', evaluation_id: evaluation.value.id }; notice.value = '服务评价已提交，感谢您的反馈。' } catch (value) { fail(value) } }
...
async function submitReport() {
  ...
  try {
    await createReportRequest(reportForm, reportRetryKey)
...
async function submitFeedback() { if (!hasPermission('feedback.create')) return; error.value = ''; notice.value = ''; try { await createFeedback({ ...feedbackForm }); Object.assign(feedbackForm, { type: 'COMPLAINT', title: '', description: '', project_id: '', expected_contact: '' }); notice.value = '反馈已提交，我们将在 24 小时内首次响应。'; if (hasPermission('feedback.read')) await load() } catch (value) { fail(value) } }
...
onMounted(load)
onBeforeUnmount(() => { cancelReportDownload(); projectExportController?.abort() })
</script>

<template>
  <div class="portal-shell">
    <header><div><strong>客户自助门户</strong><small>统一身份平台安全登录</small></div><div class="portal-account"><span>{{ accountSecurity?.account_identifier || '当前客户' }}</span><button @click="logoutPortal">退出登录</button></div></header>
    <nav aria-label="客户门户功能"><button v-if="hasPermission('project.read')" :class="{ active: section === 'projects' }" @click="navigate('projects')">我的项目</button><button v-if="hasPermission('report.read') || hasPermission('report.request')" :class="{ active: section === 'reports' }" @click="navigate('reports')">电子报告</button><button v-if="hasPermission('filing.read')" :class="{ active: section === 'filings' }" @click="navigate('filings')">等保备案</button><button v-if="canUseFeedback" :class="{ active: section === 'feedback' }" @click="navigate('feedback')">客户反馈</button><button v-if="hasPermission('account.security.manage')" :class="{ active: section === 'security' }" @click="navigate('security')">账号安全</button></nav>
    <main>
