...
function reportDownloadFailure(value) {
  const messages = {
	...
	PORTAL_REPORT_VOIDED: '该报告版本已作废，旧下载授权已撤销，请下载最新版本。',
  }
...
async function downloadReportFile() {
  ...
  if (!capabilityAvailable('report_download') || !report || report.status !== 'ISSUED' || report.report_validity_status === 'VOID' || !hasPermission('report.read') || !hasPermission('report.download') || reportDownloadLoading.value) return
...
<template>
  <div class="console-page portal-shell">
    ...
    <main class="console-main portal-main">
      ...
      <p class="portal-info" role="status">登录、会话、项目查询、报告申请和等保备案草稿均已接入门户服务；客户反馈与账号安全也使用 Portal 服务端会话。项目快照和已发放报告支持短效受控 PDF 下载；备案材料已接入受控上传与文件校验状态，文件网关未配置时失败关闭，备案 PDF 和公安提交仍未接通。</p><p v-if="error" class="portal-error" role="alert">{{ error }}</p><p v-if="notice" class="portal-success" role="status">{{ notice }}</p><p v-if="loading">正在加载…</p>
