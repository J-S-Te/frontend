...
import { listPositions, listUsers } from '../api/iam.js'
...
const form = reactive({ userId: '', type: 'TRANSFER', fromPositionId: '', toPositionId: '', reason: '', approvalNo: '', effectiveDate: '' })
...
function resetForm() { Object.assign(form, { userId: '', type: 'TRANSFER', fromPositionId: '', toPositionId: '', reason: '', approvalNo: '', effectiveDate: '' }); preview.value = null }

function payload() {
  return { user_id: form.userId, change_type: form.type, source_membership_id: form.fromPositionId || null, target_position_id: form.toPositionId || null, reason: form.reason, approval_reference: form.approvalNo || null, effective_at: form.effectiveDate ? `${form.effectiveDate}T00:00:00+08:00` : null }
}
...
<template>
  <section class="console-card settings-card personnel-change-center">
    <div class="console-card-body">
      ...
      <div v-else class="personnel-change-table"><div class="personnel-change-row personnel-change-header"><span>人员</span><span>异动类型</span><span>状态</span><span>生效日期</span><span>审批单号</span><span>操作</span></div><div v-for="item in visibleRecords" :key="item.id || item.change_id" class="personnel-change-row"><span>{{ item.user_name || item.display_name || item.user_id || '—' }}</span><span>{{ typeLabel(item.change_type || item.type) }}</span><span><span class="console-badge">{{ statusLabels[item.status] || item.status || '—' }}</span></span><span>{{ item.effective_at || item.effective_date || '—' }}</span><span class="console-mono">{{ item.approval_reference || item.approval_no || '—' }}</span><span class="personnel-change-actions"><button class="console-button compact" type="button" @click="openAuthorization(item)">授权概览</button><button v-if="item.status === 'DRAFT'" class="console-button compact" type="button" @click="submit(item.id || item.change_id)">提交审批</button></span></div></div>
    </div>
  </section>
  <div v-if="showForm" class="console-modal-backdrop" role="presentation" @click.self="showForm = false">
    <section class="console-detail-modal personnel-change-modal" role="dialog" aria-modal="true" aria-label="新建人员异动单"><header><div><p class="console-modal-eyebrow">PERSONNEL CHANGE</p><h2>新建人员异动单</h2></div><button class="console-modal-close" type="button" @click="showForm = false">×</button></header>
      <div class="console-form-grid"><label class="console-form-item"><span>人员 *</span><select v-model="form.userId"><option value="">请选择人员</option><option v-for="item in users" :key="item.id || item.user_id" :value="item.id || item.user_id">{{ userLabel(item) }}</option></select></label><label class="console-form-item"><span>异动类型 *</span><select v-model="form.type"><option v-for="[key, label] in typeOptions" :key="key" :value="key">{{ label }}</option></select></label><label class="console-form-item"><span>原岗位</span><select v-model="form.fromPositionId"><option value="">请选择</option><option v-for="item in positions" :key="item.id || item.position_id" :value="item.id || item.position_id">{{ item.name || item.position_name || item.code }}</option></select></label><label class="console-form-item"><span>新岗位</span><select v-model="form.toPositionId"><option value="">请选择</option><option v-for="item in positions" :key="item.id || item.position_id" :value="item.id || item.position_id">{{ item.name || item.position_name || item.code }}</option></select></label><label class="console-form-item"><span>审批单号</span><input v-model="form.approvalNo" placeholder="可选" /></label><label class="console-form-item"><span>生效日期 *</span><input v-model="form.effectiveDate" type="date" /></label><label class="console-form-item full"><span>变更原因 *</span><textarea v-model="form.reason" rows="3" placeholder="填写业务原因、交接说明或复职依据" /></label></div>
