...
test('平台设置页签只由各自后端权限决定', () => {
  ...
  assert.deepEqual(visiblePlatformSettingsSections(principal('platform:settings:read')), ['base'])
...
test('通配权限可访问所有设置页签', () => {
  assert.deepEqual(visiblePlatformSettingsSections(principal('*')), ['base', 'iam', 'notify', 'security', 'dict', 'applications'])
