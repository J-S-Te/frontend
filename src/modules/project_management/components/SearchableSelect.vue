...
<template>
  <div ref="root" class="pm-search-select" :class="{ open, disabled, multiple }" @keydown="onKeydown">
    ...
    <div v-if="open" class="pm-search-select-menu" @click.stop>
      ...
      <div class="pm-search-select-options" role="listbox" :aria-label="ariaLabel" :aria-multiselectable="multiple">
        <button
          ...
          :class="{ active: activeIndex === index, selected: isSelected(option), disabled: option.disabled }"
          role="option"
        ...
        >
          ...
          <span><b>{{ optionLabel(option) }}</b><small v-if="option[descriptionKey]">{{ option[descriptionKey] }}</small></span>
        </button>
    ...
    <div v-if="multiple && selectedOptions.length" class="pm-search-select-chips">
      <span v-for="option in selectedOptions" :key="String(optionValue(option))">
        {{ optionLabel(option) }}
...
.pm-search-select-trigger { width: 100%; min-height: var(--pm-h-control); display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 0 13px; border: 1px solid var(--pm-line); border-radius: var(--pm-r-sm); background: var(--pm-card); color: var(--pm-ink); text-align: left; cursor: pointer; font: inherit; }
...
.pm-search-select-menu { position: absolute; z-index: var(--pm-z-dropdown); top: calc(100% + 6px); left: 0; right: 0; min-width: min(420px, 86vw); padding: 8px; border: 1px solid var(--pm-line); border-radius: var(--pm-r-md); background: var(--pm-card); box-shadow: var(--pm-shadow-pop); }
.pm-search-select-search { display: flex; align-items: center; gap: 8px; padding: 0 10px; border: 1px solid var(--pm-line); border-radius: var(--pm-r-sm); background: var(--pm-sunken); }
.pm-search-select-search input { width: 100%; min-height: 36px; border: 0; outline: 0; background: transparent; color: inherit; font: inherit; }
.pm-search-select-options { max-height: 240px; margin-top: 7px; overflow: auto; overscroll-behavior: contain; }
...
@media (max-width: 720px) { .pm-search-select-menu { min-width: 100%; } }
</style>
