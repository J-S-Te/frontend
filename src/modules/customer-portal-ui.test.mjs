...
test('备案向导具备七步、权限门禁、冲突停止、全量校验与内部锁定语义', () => {
  ...
  for (const permission of ['filing.create', 'filing.update', 'filing.submit']) assert.match(filingWizard, new RegExp(permission.replace('.', '\\.')))
...
test('备案矩阵使用原生单选，材料走受控上传且 PDF 能力失败关闭', () => {
  ...
  assert.match(filingWizard, /matrix version=/)
