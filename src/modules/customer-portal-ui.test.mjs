...
const portalStyles = await readFile(new URL('./customer_portal/styles/customer-portal.css', import.meta.url), 'utf8')
...
test('Portal 页面已对齐真实 HTTP 路由并对未配置能力失败关闭', () => {
  ...
  assert.doesNotMatch(view, /前往统一身份账号安全中心/)
  assert.doesNotMatch(view, /用户名|忘记密码|密码强度/)
  assert.match(portalStyles, /\.filing-list-open strong \{[^}]*visibility: visible;[^}]*opacity: 1;/s)
  assert.match(portalStyles, /\.filing-list-open \{[^}]*transition: none !important;/s)
...
test('备案向导具备七步、权限门禁、冲突停止、全量校验与内部锁定语义', () => {
  ...
  assert.match(filingWizard, /function filingTitle\(item\)/)
  assert.match(filingWizard, /\.join\(' - '\)/)
  assert.match(filingWizard, /<strong>\{\{ filingTitle\(item\) \}\}<\/strong>/)
  assert.match(filingWizard, /<h2>\{\{ filingTitle\(filing\) \}\}<\/h2>/)
  assert.doesNotMatch(filingWizard, /<h2>\{\{ filing\.filing_no \}\}<\/h2>/)
  assert.match(filingWizard, /filing\.value\.unit_name \|\|=/)
  assert.match(filingWizard, /filing\.value\.system_name \|\|=/)
  ...
  assert.match(filingWizard, /const savingNow = ref\(false\)/)
  assert.match(filingWizard, /savingNow\.value = true/)
  assert.match(filingWizard, /已暂存/)
  assert.match(filingWizard, /暂存中…/)
  assert.match(filingWizard, /async function nextStep\(\)/)
  assert.match(filingWizard, /async function goToStep\(index\)/)
  assert.match(filingWizard, /@click="goToStep\(index\)"/)
  assert.match(filingWizard, /if \(issues\.length\) showValidationNotice\(issues\)/)
  assert.match(filingWizard, /if \(issues\.length\) showValidationNotice\(issues\)[\s\S]{0,120}currentStep\.value \+= 1/)
  assert.match(filingWizard, /validation\.value = await validateFiling\(filing\.value\.id\)/)
  assert.match(filingWizard, /@click="nextStep"/)
  assert.doesNotMatch(filingWizard, /sectionIssues\[code\] = saved\.validation_issues \|\| \[\]\s*showValidationNotice\(sectionIssues\[code\]\)/s)
  assert.match(filingWizard, /function showValidationNotice\(issues\)/)
  assert.match(filingWizard, /function dismissValidationNotice\(\)/)
  assert.match(filingWizard, /aria-label="关闭校验提示"/)
  assert.match(filingWizard, /@click="dismissValidationNotice"/)
  assert.match(filingWizard, /setTimeout\(\(\) => \{ validationNotice\.value = \[\] \}, 5000\)/)
  assert.match(filingWizard, /filing-validation-popover/)
  assert.match(filingWizard, /备案完整性检查/)
  assert.match(filingWizard, /校验通过.*待处理/s)
  assert.match(filingWizard, /发现 \{\{ validation\.issues\?\.length \|\| 0 \}\} 项待处理问题/)
  assert.match(filingWizard, /filing-validation-heading/)
  assert.doesNotMatch(filingWizard, /issue\.message \}（\{\{ issue\.path \}\}）/)
  assert.match(filingWizard, /function snapshotSection\(code\)/)
  assert.doesNotMatch(filingWizard, /structuredClone\(sectionData\[code\]\)/)
...
test('客户门户表单受控化、分页加载与流程引导', () => {
  ...
  assert.doesNotMatch(view, /securityCenterHref/)
