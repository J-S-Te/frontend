...
const currentUserLabel = computed(() => session.value?.username || session.value?.user_name || session.value?.display_name || '当前用户')
...
function exportContracts() {
  ...
  const csv = [['合同编号', '合同名称', '合同类型', '服务类型', '金额', '负责人用户名', '状态'], ...rows]
...
function normalizeContract(item) {
  return {
    ...
    owner: item.owner_username || (item.owner_user_id === session.value?.user_id ? currentUserLabel.value : '未知用户'),
...
function usernameFor(userId, username) {
